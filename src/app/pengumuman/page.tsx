...
import { Megaphone, Calendar, FileText, X, Search } from "lucide-react";
          ...
          <div className="space-y-4">
            {pengumumanTersaring.length > 0 ? (
              ...
              <div className="text-center py-12 bg-white border border-slate-200 rounded-2xl shadow-sm">
                <p className="text-slate-400 text-xs sm:text-sm font-medium">Tidak ada data pengumuman yang cocok.</p>
              </div>
            )}
          </div>
            ...
            <div className="fixed inset-0 bg-slate-950/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
              <div className="bg-white w-full max-w-xl rounded-2xl shadow-2xl overflow-hidden max-h-[85vh] flex flex-col border border-slate-100">
                <div className="p-4 bg-slate-50 border-b border-slate-200 flex justify-between items-center">
                  ...
                  <button onClick={() => setPengumumanTerpilih(null)} className="p-1.5 text-slate-400 hover:text-slate-600 hover:bg-slate-200/60 rounded-xl transition-all cursor-pointer">
                    <X size={16} />
                  </button>
                </div>
